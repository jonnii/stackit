...
func TestConfigCommand(t *testing.T) {
	...
	t.Run("config get create.ai returns false by default", func(t *testing.T) {
		t.Parallel()
		scene := testhelpers.NewSceneParallel(t, nil)
		...
		// Get create.ai (should return false by default)
		cmd = exec.Command(binaryPath, "config", "get", "create.ai")
		cmd.Dir = scene.Dir
		output, err := cmd.CombinedOutput()
		require.NoError(t, err, "config get command failed: %s", string(output))
	...
	t.Run("config set and get create.ai", func(t *testing.T) {
		t.Parallel()
		scene := testhelpers.NewSceneParallel(t, nil)
		...
		// Set create.ai to true
		cmd = exec.Command(binaryPath, "config", "set", "create.ai", "true")
		cmd.Dir = scene.Dir
		output, err := cmd.CombinedOutput()
		require.NoError(t, err, "config set command failed: %s", string(output))
		require.Contains(t, string(output), "Set create.ai to:")
		...
		// Set create.ai to false
		cmd = exec.Command(binaryPath, "config", "set", "create.ai", "false")
		cmd.Dir = scene.Dir
		output, err = cmd.CombinedOutput()
		require.NoError(t, err, "config set command failed: %s", string(output))
	...
	t.Run("config set create.ai rejects invalid boolean values", func(t *testing.T) {
		t.Parallel()
		scene := testhelpers.NewSceneParallel(t, nil)
		...
		// Try to set create.ai to invalid value
		cmd = exec.Command(binaryPath, "config", "set", "create.ai", "yes")
		cmd.Dir = scene.Dir
		output, err := cmd.CombinedOutput()
		require.Error(t, err, "config set should fail with invalid boolean value")
		require.Contains(t, string(output), "invalid value for create.ai")

		// Verify it was not set (should still be false)
		cmd = exec.Command(binaryPath, "config", "get", "create.ai")
		cmd.Dir = scene.Dir
		output, err = cmd.CombinedOutput()
		require.NoError(t, err)
	...
	t.Run("config set create.ai persists across commands", func(t *testing.T) {
		t.Parallel()
		scene := testhelpers.NewSceneParallel(t, nil)
