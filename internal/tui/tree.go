...
func (r *StackTreeRenderer) getInfoLines(args treeRenderArgs) []string {
	...
	if scope == "" && args.indentLevel > 0 && args.indentLevel-1 < len(args.parentScopes) {
		// Fallback to parent scope for tree characters if current has no scope?
		// User said "color coding by scope", usually means the branch's scope.
	}
	style := lipgloss.NewStyle().Foreground(GetScopeColor(scope))
	if isDim {
		style = style.Copy().Foreground(lipgloss.Color("8"))
	}
